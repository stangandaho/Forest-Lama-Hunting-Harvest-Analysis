...
colnames(tech_chasse_avant) <- gsub("Techniques de chasse_|_Avant|_", "", colnames(tech_chasse_avant) )
tca_list <- list()
...
colnames(tech_chasse_auj) <- gsub("Techniques de chasse_|_Aujourd'hui|_", "", colnames(tech_chasse_avant) )
tcauj_list <- list()
...
tech_chasse_tbl %>% 
  ...
  scale_fill_manual(values = color_plt[c(4, 5)])+
  labs(x = "Techniques de chasse", fill = "")+
  theme(
    # axis
    ...
    # strip
    strip.background = element_rect(fill = color_plt[1]),
    strip.text = element_text(colour = color_plt[5], 
                              family = "msb", size = 25,
...
ggsave(filename = "plots/OS1_techniques_de_chasse.jpeg", width = 20, height = 12, dpi = 100)
